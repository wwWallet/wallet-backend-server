...
/**
 * Helper function to look up the authenticator name from either the FIDO or non-FIDO cache.
 */
function resolveAuthenticatorName(aaguid: string | null | undefined): string {
	if (!aaguid) return "Unknown Authenticator";

	const fidoEntry = memoryCache.data?.[aaguid];
	if (fidoEntry) {
		const fidoName = fidoEntry.friendlyNames?.["en-us"] || fidoEntry.description;
		if (fidoName) return fidoName;
	}

	const nonFidoName = memoryCache.nonFidoData?.[aaguid]?.name;
	if (nonFidoName) return nonFidoName;
...
if (!config.registerDisabled) {
	...
	noAuthUserController.post('/register-webauthn-finish', async (req: Request, res: Response) => {
		...
		if (verification.verified) {
			...
			const authenticatorName = resolveAuthenticatorName(aaguid);
	...
	res.status(200).send({
		...
		webauthnCredentials: (user.webauthnCredentials || []).map(cred => {
			return {
				...
				aaguid: cred.aaguid,
				authenticatorName: resolveAuthenticatorName(cred.aaguid),
			};
		}),
	});
})
