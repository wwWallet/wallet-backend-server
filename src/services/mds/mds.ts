...
import { verifyCertificateChain } from './crypto';

interface FidoJwtHeader {
	alg: string;
	x5c?: string[];
}

interface FidoMdsEntry {
	aaguid?: string;
	metadataStatement: any;
	[key: string]: any;
}

interface FidoJwtPayload {
	nextUpdate: string;
	entries: FidoMdsEntry[];
	[key: string]: any;
}

export interface MdsParsedData {
	payloadMap: Record<string, any>;
	nextUpdate: string;
}

export async function validateBlobAuthenticity(rawBlob: string): Promise<void> {
	const parts = rawBlob.trim().split('.');
	if (parts.length !== 3) throw new Error("Invalid JWT BLOB format.");

	const header: FidoJwtHeader = JSON.parse(Buffer.from(parts[0], 'base64url').toString('utf8'));
...
export function parseAndFormatBlob(rawBlob: string): MdsParsedData {
	const parts = rawBlob.trim().split('.');
	const payload: FidoJwtPayload = JSON.parse(Buffer.from(parts[1], 'base64url').toString('utf8'));
