...
export async function verifyCertificateChain({ trustedRoot, leaf, intermediates = [] }: VerifyChainParams): Promise<boolean> {
    try {
        ...
        const builtRootBuffer = await builtRoot.getThumbprint();
        const rootCaBuffer = await rootCa.getThumbprint();
        const builtRootHex = Buffer.from(builtRootBuffer).toString('hex');
        const rootCaHex = Buffer.from(rootCaBuffer).toString('hex');

        if (builtRootHex !== rootCaHex) {
